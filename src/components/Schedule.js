            ...
            {schedules && 
              schedules.map((schedule, index) => {
                return (
                  <div
                    key={index}
                    className="shadow-md bg-white my-1 px-10 py-10 flex flex-row items-center gap-4"
                  >
                    <BsPersonCircle className="text-4xl" />
                    <h4 className="font-bold text-2xl text-black">
                      {schedule.username[0].toUpperCase() +
                        schedule.username.slice(1, schedule.username.length)}
                    </h4>
                  </div>
                );
              })}
              {schedules?.length === 0 && (<p className="my-4">No schedule found for this day</p>)}
          </div>
        </div>
      </div>
    </>
